...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        i = 0
        while place != beehive:
            if place.bees and self.min_range <= i <= self.max_range:
...
class ContainerAnt(Ant):
    ...
    def action(self, gamestate):
        # BEGIN Problem 9
        "*** YOUR CODE HERE ***"
        if self.contained_ant and self.armor:
            self.contained_ant.action(gamestate)
        elif self.armor <= 0:
            ContainerAnt.remove_from(self, self.place)
        # END Problem 9
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, gamestate):
        ...
        elif not self.imposter:
            ScubaThrower.action(self, gamestate)
            place = self.place
            while place.exit:
                ant = place.exit.ant
                if ant and (ant not in self.have_doubled):
                    ant.damage *= 2
                    if isinstance(ant, ContainerAnt) and ant.contained_ant:
                        if ant.contained_ant not in self.have_doubled:
                            ant.contained_ant.damage *= 2
                            self.have_doubled.append(ant.contained_ant)
                    self.have_doubled.append(ant)
                elif isinstance(ant, ContainerAnt) and ant.contained_ant:
                    if ant.contained_ant not in self.have_doubled:
                        ant.contained_ant.damage *= 2
                        self.have_doubled.append(ant.contained_ant)
                place = place.exit
    ...
    def reduce_armor(self, amount):
        ...
        if not self.imposter and self.armor <= 0:
            bees_win()
        elif self.imposter and self.armor <= 0:
            self.place.remove_insect(self)
            self.death_callback()
...
#                 if cur_place.exit.ant and ant not in self.have_doubled:
#                     ant.damage *= 2
#                     self.have_doubled.append(ant)
...
#             bees_win()
#         elif self.armor <= 0 and self.imposter:
#             self.place.remove_insect(self)
#             self.death_callback()
#
#     def remove_from(self, place):
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem 7
        return False if not self.place.ant or (self.place.ant and not self.place.ant.blocks_path) else True
        # END Problem 7
